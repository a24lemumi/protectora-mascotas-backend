...
- **WHEN** an authenticated user sends POST to `/api/mascotas/{id}/adoptar`
...
The system SHALL have a protected POST route `/api/mascotas/{id}/adoptar` with JwtMiddleware.

#### Scenario: Route registered in index.php
- **WHEN** checking `public/index.php` route definitions
- **THEN** there is a POST route `/api/mascotas/{id}/adoptar` with JwtMiddleware
